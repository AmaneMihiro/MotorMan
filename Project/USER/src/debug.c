...
void Roundabout_debugshow(void)
{
	...
	ips114_showuint8(0,4,Right_Adc+Left_Adc);     
    ips114_showuint8(0,5,Left_Shu_Adc+Right_Shu_Adc);     
    ips114_showuint8(0,6,Right_Adc+Left_Adc+Left_Shu_Adc+Right_Shu_Adc);     
	ips114_showfloat(0,7,Current_Dir,2,1);//显示浮点数   整数显示2位   小数显示1位	
	
	ips114_showuint8(50,0,road_type.annulus);         //环岛标志位
    ips114_showuint8(50,1,road_type.in_annulus_left); //左环岛进环打角标志位
	ips114_showuint8(50,2,road_type.in_annulus_right); //左环岛进环打角标志位
	ips114_showuint8(50,3,road_type.on_annulus_left); //在左环岛标志位（认为小车已经入环）
	ips114_showuint8(50,4,road_type.on_annulus_right); //在右环岛标志位（认为小车已经入环）
	ips114_showuint8(50,5,road_type.out_annulus);     //出环岛标志位

	ips114_showint16(100,1,annulus_s);                 //过环岛三角区编码器积分距离
	ips114_showint16(100,2,annulus_s2);                 
