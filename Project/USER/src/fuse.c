...
void Motor_output_control()
{
	...
	// 待修改差速参数
	Speed_pwm_left += IncPIDCalc(&L_SpeedPID, aim_speed + ADC_PWM, left_real_speed);
	Speed_pwm_right += IncPIDCalc(&R_SpeedPID, aim_speed - ADC_PWM, right_real_speed);

	//	  Speed_pwm_left =range_protect(Speed_pwm_left,  left_real_speed,  -aim_speed, 2*aim_speed);//减速限幅（防止轮子反转太大导致假期望速度）
	//	  Speed_pwm_right=range_protect(Speed_pwm_right, right_real_speed, -aim_speed, 2*aim_speed);//减速限幅（防止轮子反转太大导致假期望速度）
...
返回值：duty
**************************************************************************************/
// int16 range_protect(int16 input, int16 min, int16 max)//限幅保护
//{
